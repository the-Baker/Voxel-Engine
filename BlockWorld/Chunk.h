...
glm::ivec3 chunkToWorldPosition(glm::ivec3 blockPosInChunk, glm::ivec2 chunkPos)
{
	glm::ivec3 result;
	result.x = blockPosInChunk.x + (chunkPos.x * CHUNK_SIZE);
	result.y = blockPosInChunk.y;
	result.z = blockPosInChunk.z + (chunkPos.y * CHUNK_SIZE);
	return result;
}
